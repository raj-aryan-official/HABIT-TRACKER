...
const habitTimeInput = document.getElementById('habitTime'); // Time spent input
...
if (addHabitForm) {
  addHabitForm.addEventListener('submit', (e) => {
    ...
    const timeSpent = habitTimeInput.value;  // Get time spent value

    const newHabit = {
      id: Date.now(),
      name,
      description,
      timeSpent,  // Store the time spent
    };
...
function editHabit(id) {
  ...
  if (habit) {
    ...
    const newTimeSpent = prompt('Edit Time Spent (in hours):', habit.timeSpent);

    if (newName) habit.name = newName;
    if (newDescription) habit.description = newDescription;
    if (newTimeSpent) habit.timeSpent = newTimeSpent;
